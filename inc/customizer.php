 ...
 * @return void
 */
function mediafast_customize( $wp_customize ) {
	/**
	 * Initialize sections
	 */
	...
	$wp_customize->add_control(
		new WP_Customize_Image_Control(
			...
			'header_logo',
			array(
				'label'       => __( 'Upload Header Logo', 'mediafast' ),
				'description' => __( 'Height: &gt;80px', 'mediafast' ),
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'radio',
			'label'   => __( 'Navbar Scheme', 'mediafast' ),
			'section' => 'theme_header_section',
			'choices' => array(
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'radio',
			'label'   => __( 'Navbar', 'mediafast' ),
			'section' => 'theme_header_section',
			'choices' => array(
				'static'       => __( 'Static', 'mediafast' ),
				'fixed_top'    => __( 'Fixed to top', 'mediafast' ),
				'fixed_bottom' => __( 'Fixed to bottom', 'mediafast' ),
			),
		)
	);
	...
	$wp_customize->add_control(
		...
		array(
			'type'    => 'checkbox',
			'label'   => __( 'Show Searchfield?', 'mediafast' ),
			'section' => 'theme_header_section',
		)
	);
}
add_action( 'customize_register', 'mediafast_customize' );
 ...
 * @return void
 */
function mediafast_customize_preview_js() {
	wp_enqueue_script( 'customizer', get_template_directory_uri() . '/inc/customizer.js', array( 'jquery' ), null, true );
}
add_action( 'customize_preview_init', 'mediafast_customize_preview_js' );
